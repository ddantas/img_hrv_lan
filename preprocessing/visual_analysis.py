...
def plot_inferred_vs_real_rr(rr_values, inferred_rr_values, ecg_values):
	...
	rr = [v*1.024/(1000) for v in rr_values[1]]
	rr = np.array([sum(rr[:i]) for i in range(len(rr))])
	rr += time_intervals[rpeaks[0]]
	i_rr = [v*1.024/(1000) for v in inferred_rr_values[1]]
	...
	axs[2].plot(x[:521], derivative_rr)
	axs[2].plot(x[:521], derivative_i_rr)
...
if __name__ == '__main__':
	...
	rr_values = construct_xy_from_file('../data/a003/subj1_rr.tsv')

	infer_rr_intervals_from_ecg('../data/a003/subj1_ecg.tsv')
	inferred_rr_values = construct_xy_from_file('../data/a003/processed/subj1_rr_inferred_from_ecg.tsv')
	ecg_values = construct_xy_from_file_ecg('../data/a003/subj1_ecg.tsv')
